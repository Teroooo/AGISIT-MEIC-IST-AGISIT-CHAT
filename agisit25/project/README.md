...
# AGISIT Project Specification

## Introduction
This project will require a hands-on infrastructure environment that will allow you to validate the skills and knowledge you have learned so far in this course. It corresponds to 50% of the course's final grade.

The Project is sized to be developed by a team of (preferably) 3 students. The objective, although simple, corresponds to a **realistic** example on how to deploy and provision a tiered (frontend, backend) **Microservices-based** containerized Web Application on a Public Cloud provider, such as **Google Cloud Platform** (GCP) or **Amazon Web Services** (AWS). The project entails the use of automation tools such as **Terraform**, **Ansible**, **Pulumi**, as well as the instrumentation of applications, services and infrastructure components to monitor and log application behavior using tools such as **Prometheus**, **Fluentd**, **Grafana**, etc.
...
**Step 1**: Building a simple Web Application using a Microservices-based architecture. See below the specification of the target application that should be developed;

**Step 2**: Deploying the required infrastructure locally as containers, using Docker Compose for example;

**Step 3**: Deploying the infrastructure on a Kubernetes cluster. The Kubernetes cluster should be launched entirely using IaC tools. The application should also be deployed using IaC tools. Only self-hosted Kubernetes deployments are allowed, solutions such as Google Kubernetes Engine are forbidden;

**Step 4**: Deploying operations tooling that will allow basic traceability and performance monitoring of the solution. For that, you will need to have configured the application (and other resources) to expose some metrics in order to show their health and status, and then deploy a monitoring server using, for example Prometheus, and create a Dashboard (e.g., using Grafana) to display the metrics in real-time.
...
The advanced components for the project either provide additional features or correspond to different deployment methods. The implementation of these features has a **weight of 30%** on the evaluation of the project. The maximum grade of the project can be reached by implementing **one** the following options in your project:

- Create and run a **Continuous Integration** (CI) / **Continuous Deployment** (CD) pipeline for the microservice application, using for example Jenkins, in order to automatically test, package and deploy the application on every commit;
- Deploy the developed microservice application with **persistent state** using self-hosted storage and or databases. With this feature, you should be able to shutdown the entire infrastructure and later turn it back on without loosing any application state;
- Deploy the microservice application with **automatic scaling**, meaning that you should automatically scale up and down the number of workers depending on the load being received by the application.

## Target Application Domain

For this year's project, the application is an instant messaging application (similar to WhatsApp, Skype, etc). The goal is that users can register in the system, login, and then can send messages to other users. The developed platform should allow users to not only exchange messages directly but to also have support for group chats. It should also support holding messages to users that are not online when the message is sent and notifying them of the messages when they come back online.

The project is open-ended and we do not specify features that the application should have as long as it can be used to exchange messages among users without crashing.

The application should expose a web interface that could be used from a normal browser. Focus on functionality rather than aesthetics.
...
**The deadline for this submission is Friday, October 24th, 2025, by 11h59 pm.**
...
- **Gitlab**: the last commit of the project pushed to the group Git repository, `https://gitlab.rnl.tecnico.ulisboa.pt/agisit/agisit25-gN`, where `N` represents your group number;
...
The video can be structured in any way. It can be an interactive tour, a demo, a slideshow with voice overlay, etc. It constitutes an opportunity to better explain how the application was developed, how it is implemented, the main hurdles encountered, and above all to demonstrate that it works! If you implemented any advanced feature, you should show this feature working during your video demonstration.
...
These open-source applications should be used as a source of inspiration. **You should build your own application and not use these applications directly.**
